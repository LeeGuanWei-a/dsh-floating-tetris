...
function LaunchButton(props) {
  ...
  const style = {
    ...
    color: 'inherit', fontSize: 15, lineHeight: 1, padding: '4px 6px',
    borderRadius: 6, opacity: open ? 1 : 0.7
  }
  return el('button', { style, title: open ? '收起俄罗斯方块' : '打开俄罗斯方块', onClick: () => store.toggle() }, '🎮')
}
...
export function apply(ctx) {
  ...
  // Entry lives in the session header's right-aligned utilities row (an icon
  // strip), NOT in sidebar.footer.action: that sidebar row is a horizontal
  // line shared with other plugins and gets crowded/overlapped.
  slots.inject('conversation.session.header.utilities', () => slots.register(
    { name: 'conversation.session.header.utilities', id: 'tetris-launch', order: 20, label: '俄罗斯方块' },
    () => el(LaunchButton, { store })
  ))
